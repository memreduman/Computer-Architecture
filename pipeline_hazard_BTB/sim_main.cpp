...
#define flush					top__DOT__u1__DOT__Branch_Prediction_Miss
#define brach_predicted			top__DOT__u1__DOT__Branch_Predicted
...
public:
  ...
  void tick(void) { 
	...
	if (DEBUG) {
	  ...
	  printf("\nStall = %d"		,m_top->stall);
	  printf("\nFlush = %d"		,m_top->flush);
	  printf("\nBranch Predicted = %d"		,m_top->brach_predicted);
...
int main(int argc, char** argv, char** env){
  ...
  while(1) // 5-stage pipeline, so each instruction will be executed maximum within 5 clock-cycle
  {
	   ...
	   if(tb->m_top->PC==13*4 && !tb->m_top->stall){
		   break;
	   }
  }

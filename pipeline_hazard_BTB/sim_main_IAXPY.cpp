...
#define flush					top__DOT__u1__DOT__flush

#define Branch_Prediction_Miss		top__DOT__u1__DOT__Branch_Prediction_Miss
#define Branch_Predicted			top__DOT__u1__DOT__Branch_Predicted
#define Branch_PC_Miss				top__DOT__u1__DOT__Branch_PC_Miss
...
public:
  ...
  void tick(void) { 
	...
	if (DEBUG) {
	  printf("\n %ld Clock cycle\n",m_tickcount);
      printf("PC=%d\n",m_top->PC);
      printf("InstData=%08x\n",m_top->top__DOT__u1__DOT__instmem_data);
	  printf("RF Write Data=%x\n",m_top->top__DOT__u1__DOT__RF_WriteData);
	  ...
	  printf("\nStall = %d"		,m_top->stall);
	  printf("\nFlush = %d"		,m_top->flush);
	  printf("\nBranch Predicted = %d"		,m_top->Branch_Predicted);
	  printf("\nBranch Prediction Miss = %d"		,m_top->Branch_Prediction_Miss);
	  printf("\nBranch PC Miss = %d"		,m_top->Branch_PC_Miss);
...
int main(int argc, char** argv, char** env){
  ...
  tb->opentrace("waveforms_emre.vcd");
  ...
  // Verify 
	unsigned int stall_cycle=0;
	unsigned int flush_counter=0;
	unsigned int Branch_PC_miss=0;
	unsigned int Branch_Prediction_miss=0;
	unsigned int Branch_Prediction_counter=0;
  ////////////////////////////////// 
  ...
  while(1) // 5-stage pipeline, so each instruction will be executed maximum within 5 clock-cycle
  {
	   ...
	   if(tb->m_top->top__DOT__u1__DOT__Branch_PC_Miss)				Branch_PC_miss++;
	   if(tb->m_top->top__DOT__u1__DOT__Branch_Prediction_Miss)		Branch_Prediction_miss++;
	   if(tb->m_top->top__DOT__u1__DOT__Branch_Predicted)		Branch_Prediction_counter++;
  ...
  printf("\n Flush Counter = %d\n",flush_counter);
  printf("\n Branch PC miss = %d\n",Branch_PC_miss);
  printf("\n Branch Prediction Miss = %d\n",Branch_Prediction_miss);
  printf("\n Branch Prediction counter = %d\n",Branch_Prediction_counter);
  printf("------ ----------------------- ------\n");
  int value[25];
  for(int i = 1; i <= 25 ; i++){
		  value[i-1] = i*100 + i*2;
